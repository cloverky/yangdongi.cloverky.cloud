  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950">
      <header className="flex items-center justify-between px-5 py-3 border-b border-gray-800 shrink-0">
        ...
        <Link href="/login" className="text-xs text-teal-400 hover:underline">로그인하기</Link>
      </header>

      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        <div className="mx-auto max-w-md bg-gray-800 rounded-xl p-4 text-sm text-gray-400 text-center">
          게스트 모드에서는 대화 내역이 저장되지 않습니다.<br/>
          <Link href="/login" className="text-teal-400 hover:underline">로그인</Link>하면 학번 연동 기능을 사용할 수 있습니다.
        </div>

        {messages.length === 0 && (
          <div className="text-center text-gray-500 mt-10">
            <p className="font-medium">동양미래대학교에 대해 무엇이든 물어보세요</p>
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {["입학 전형 안내해줘", "캠퍼스 위치 어디야?", "학과 목록 알려줘", "학교 시설 뭐 있어?"].map((q) => (
